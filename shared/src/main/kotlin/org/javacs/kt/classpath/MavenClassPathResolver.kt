...
internal class MavenClassPathResolver private constructor(private val pom: Path) : ClassPathResolver {
    ...
    override val classpath: Set<ClassPathEntry> get() {
        ...
        return async.ioMap(artifacts) { findMavenArtifact(it, false)?.let { it1 -> ClassPathEntry(it1, null) } }
            .filterNotNull()
            .toSet()
    }
    ...
    override val currentBuildFileVersion: Long get() = BuildFileHashing.hash(pom)
...
private fun resolveMavenDependencies(pom: Path) {
    val command = listOf(mvnCommand(pom).toString(), "dependency:resolve", "-DincludeScope=test")
...
private fun runCommand(pom: Path, command: List<String>) {
    ...
    if ("BUILD FAILURE" in errors) {
        throw KotlinLSException("Maven task failed: ${errors.lines().firstOrNull()}")
    }
}
