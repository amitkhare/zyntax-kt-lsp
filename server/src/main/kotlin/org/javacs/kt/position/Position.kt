...
fun offset(content: String, line: Int, char: Int): Int {
    check(!content.contains('\r')) { "Content contains \\r characters which break offset calculation" }
...
/**
 * Computes a [Location] for the given PSI element.
 *
 * The location consists of the URI of the file containing [expr] and the character range of the element
 * within that file.
 *
 * If the file content cannot be accessed (for example, due to a [NullPointerException] from the PSI),
 * this function returns `null`.
 *
 * @param expr the PSI element whose location should be determined
 * @return a [Location] representing the file URI and text range of [expr], or `null` if the file content
 * is unavailable
 */
fun location(expr: PsiElement): Location? {
    // Binary class PSI (e.g. jrt:// JDK modules) has no source text, and getText() asserts on it.
    // That AssertionError marks an unexpected internal state, but here it's benign and expected: discarding it
    // returns null so callers fall back to the source lookup instead of crashing the hover.
    val content = try {
