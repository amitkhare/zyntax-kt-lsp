...
val GRADLE_DSL_DEPENDENCY_PATTERN = Regex("^gradle-(?:kotlin-dsl|core).*\\.jar$")

private val GRADLE_DSL_IMPORTS = listOf(
...
fun findGradleDSLScriptDefinitions(classPath: Set<Path>): List<ScriptDefinition> {
    val foundDSLDependency = classPath.any { GRADLE_DSL_DEPENDENCY_PATTERN.matches(it.fileName.toString()) }
    if (!foundDSLDependency) return emptyList()
    ...
    return try {
        ...
        scriptTemplates.map { template ->
            ScriptDefinition.FromLegacy(scriptHostConfig, object : KotlinScriptDefinitionFromAnnotatedTemplate(
                scriptClassLoader.loadClass(template).kotlin,
                ...
                override val dependencyResolver: DependenciesResolver = object : DependenciesResolver {
                    override fun resolve(scriptContents: ScriptContents, environment: Environment) =
                        ResolveResult.Success(ScriptDependencies(imports = GRADLE_DSL_IMPORTS))
                }
            })
        }
    } catch (e: Exception) {
