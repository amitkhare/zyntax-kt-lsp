...
private class CompilationEnvironment(
    ...
    init {
        environment = KotlinCoreEnvironment.createForProduction(
            ...
            configuration = KotlinCompilerConfiguration().apply {
                ...
                if (scriptsConfig.enabled) {
                    ...
                    if (scriptsConfig.buildScriptsEnabled) {
                        scriptDefinitions.addAll(findGradleDSLScriptDefinitions(classPath))
                    }
        ...
        val scriptProvider = checkNotNull(ScriptDefinitionProvider.getInstance(project)) { "ScriptDefinitionProvider not found" }
...
class Compiler(
    ...
    fun createPsiFile(content: String, file: Path = Paths.get("dummy.virtual.kt"), language: Language = KotlinLanguage.INSTANCE, kind: CompilationKind = CompilationKind.DEFAULT): PsiFile {
        check(!closed) { "Compiler is closed" }
        check(!content.contains('\r')) { "Content contains \\r characters in $file" }

        val new = psiFileFactoryFor(kind).createFileFromText(file.toString(), language, content, true, false)
        checkNotNull(new.virtualFile) { "createFileFromText returned file without virtualFile for $file" }
    ...
    fun createKtExpression(content: String, file: Path = Paths.get("dummy.virtual.kt"), kind: CompilationKind = CompilationKind.DEFAULT): KtExpression {
        check(!closed) { "Compiler is closed" }
        val declaration = createKtDeclaration("val x = $content", file, kind)
        check(declaration is KtProperty) { "Expected KtProperty but got ${declaration.javaClass.simpleName} for val x = $content" }
        return checkNotNull(declaration.initializer) { "KtProperty without initializer for val x = $content" }
    }
    ...
    fun createKtDeclaration(content: String, file: Path = Paths.get("dummy.virtual.kt"), kind: CompilationKind = CompilationKind.DEFAULT): KtDeclaration {
        ...
        check(declarations.size == 1) { "${declarations.size} declarations in $content" }
        ...
        if (onlyDeclaration is KtScript) {
            ...
            check(scriptDeclarations.size == 1) { "${scriptDeclarations.size} declarations in script in $content" }
    ...
    fun psiFileFactoryFor(kind: CompilationKind): PsiFileFactory {
        check(!closed) { "Compiler is closed" }
        return PsiFileFactory.getInstance(compileEnvironmentFor(kind).environment.project)
    }
    ...
    fun compileKtExpression(expression: KtExpression, scopeWithImports: LexicalScope, sourcePath: Collection<KtFile>, kind: CompilationKind = CompilationKind.DEFAULT): Pair<BindingContext, ComponentProvider>? =
        try {
            ...
            LOG.debug("Expression analysis not supported for this context: {}", e.message)
            null
        } catch (e: IllegalStateException) {
            LOG.debug("Illegal state during expression analysis: {}", e.message)
            null
        } catch (e: Exception) {
            LOG.warn("Error analyzing expression: {}", e.message)
        ...
        outputDirectory.takeIf { codegenConfig.enabled }?.let {
            compileLock.withLock {
                ...
                val codegenFactory = JvmIrCodegenFactory(
                    compileEnv.environment.configuration,
                )
                val state = GenerationState(
                    compileEnv.environment.project,
                    module,
                    compileEnv.environment.configuration,
