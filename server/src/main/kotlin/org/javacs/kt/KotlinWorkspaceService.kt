    ...
    override fun executeCommand(params: ExecuteCommandParams): CompletableFuture<Any> {
        ...
        when (params.command) {
            JAVA_TO_KOTLIN_COMMAND -> {
                ...
                languageClient?.applyEdit(ApplyWorkspaceEditParams(WorkspaceEdit(listOf(Either.forLeft(
                    TextDocumentEdit(
                        VersionedTextDocumentIdentifier().apply { uri = fileUri },
                        listOf(Either.forLeft(TextEdit(range, kotlinCode)))
                    )
                )))))
            }
        }
    ...
    override fun didChangeConfiguration(params: DidChangeConfigurationParams) {
        ...
        settings?.get("kotlin")?.asJsonObject?.apply {
            ...
            get("scripts")?.asJsonObject?.apply {
                ...
                get("enableJdkSymbols")?.asBoolean?.let { scripts.enableJdkSymbols = it }
                sf.updateExclusions()
            }
            ...
            get("completion")?.asJsonObject?.apply {
                ...
                get("filteredTypes")?.asJsonArray?.let { arr ->
                    val newTypes = arr.map { it.asString.trim() }
                    validateFilteredTypes(newTypes)
                    completion.filteredTypes = newTypes
                }
            }
            ...
            get("cache")?.asJsonObject?.apply {
                val cache = config.cache
                get("maxSourceFiles")?.asInt?.let {
                    cache.maxSourceFiles = it
                }
                get("maxCachedTempFiles")?.asInt?.let {
                    cache.maxCachedTempFiles = it
                }
            }
            ...
            get("externalSources")?.asJsonObject?.apply {
                ...
                get("excludedPatterns")?.asJsonArray?.let { arr ->
                    externalSources.excludedPatterns = arr.map { it.asString.trim() }
                    sf.updateExclusions()
                }
                get("generatedSourceRoots")?.asJsonArray?.let { arr ->
                    externalSources.generatedSourceRoots = arr.map { it.asString.trim() }
                    sf.updateExclusions()
                }
                get("jdkSourceOverride")?.asString?.takeIf { it.isNotBlank() }?.let { cp.jdkSourceOverride = it }
            }
        }
