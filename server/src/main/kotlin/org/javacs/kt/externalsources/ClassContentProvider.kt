...
class ClassContentProvider(
    ...
    private val cachedContents = Collections.synchronizedMap(object : LinkedHashMap<String, Pair<String, String>>() {
    ...
    fun contentOf(uri: KlsURI): Pair<KlsURI, String> {
        ...
        val result = cachedContents[key]
        if (result != null) {
            val (contents, extension) = result.also { cachedContents[key] = it }
        ...
        val fromSource = tryReadContentWithFallbacks(resolvedUri)
        val fromBinary = if (fromSource == null && resolvedUri != uri) {
            LOG.info("Source JAR inner path mismatch, falling back to binary JAR: {}", describeURI(uri.fileUri))
            tryReadContentWithFallbacks(uri)
        } else null
        val (contents, extension) = (fromSource ?: fromBinary ?: throw KotlinLSException("Could not find $uri"))
            .also { cachedContents[key] = it }
        val effectiveUri = if (fromBinary != null) uri else resolvedUri
        val sourceUri = effectiveUri.withFileExtension(extension)
        return Pair(sourceUri, contents)
    }

    /** Tries reading [uri] with all known file extensions. */
    private fun tryReadContentWithFallbacks(uri: KlsURI): Pair<String, String>? =
