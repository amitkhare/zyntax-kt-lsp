...
data class KlsURI(val fileUri: URI, val query: Map<QueryParam, String>) {
    ...
    private val innerPath: String
        get() {
            val parts = fileUri.schemeSpecificPart.split(archiveType.delimiter, limit = 2)
            check(parts.size > 1) { "No delimiter '${archiveType.delimiter}' in KLS URI: $fileUri" }
            return parts[1]
        }
    ...
    /** Builds the jar: URL from [archivePath].toUri() (encodes '[' ']' etc. and preserves '+'), so '+'/bracket jar paths (#326 / #223) reach [JarURLConnection]. */
    private fun toJarURL(): URL = URI("jar:${archivePath.toUri()}${archiveType.delimiter}$innerPath").toURL()

    private fun openJarURLConnection(): JarURLConnection {
        val conn = toJarURL().openConnection()
        check(conn is JarURLConnection) { "Expected JarURLConnection but got ${conn.javaClass.name} for $fileUri" }
        return conn
    }
    ...
    fun extractToTemporaryFile(dir: TemporaryDirectory): Path = withJarURLConnection {
        val name = it.jarEntry.name.substringAfterLast("/").split(".", limit = 2)
        if (name.size < 2) LOG.warn("JAR entry without extension: {}", it.jarEntry.name)
        val tmpFile = dir.createTempFile(name[0], name.getOrNull(1)?.let { ".$it" } ?: "")
