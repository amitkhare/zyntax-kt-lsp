...
private class NotifySourcePath(private val sp: SourcePath) {
    private val files = Collections.synchronizedMap(mutableMapOf<URI, SourceVersion>())
...
class SourceFiles(
    ...
    private val externalSourcesConfig: ExternalSourcesConfiguration = ExternalSourcesConfiguration()
) {
    private val workspaceRoots = mutableSetOf<Path>()
    private var exclusions = SourceExclusions(workspaceRoots, scriptsConfig, externalSourcesConfig.excludedPatterns, externalSourcesConfig.generatedSourceRoots)
    ...
    fun addWorkspaceRoot(root: Path) {
        ...
        val sourceList = addSources.toList()
        val readResults = async.ioMap(sourceList) { uri ->
            uri to readFromDisk(uri, temporary = false)
        }

        for ((uri, content) in readResults) {
            content?.let {
                files[uri] = it
    ...
    private fun findSourceFiles(root: Path): Set<URI> {
        ...
        return SourceExclusions(listOf(root), scriptsConfig, externalSourcesConfig.excludedPatterns, externalSourcesConfig.generatedSourceRoots)
            .walkIncluded()
    ...
    fun updateExclusions() {
        exclusions = SourceExclusions(workspaceRoots, scriptsConfig, externalSourcesConfig.excludedPatterns, externalSourcesConfig.generatedSourceRoots)
...
private fun patch(sourceText: String, change: TextDocumentContentChangeEvent): String {
    ...
    // Skip replaced text
    repeat(range.end.line - range.start.line) {
        reader.readLine()
    }
