...
fun codeActions(file: CompiledFile, index: SymbolIndex, range: Range, context: CodeActionContext, filteredTypes: List<String> = emptyList()): List<Either<Command, CodeAction>> {
    ...
    return requestedKinds.map {
        when (it) {
            ...
            CodeActionKind.QuickFix -> getQuickFixes(file, index, range, context.diagnostics, filteredTypes)
            else -> listOf()
        }
    }.flatten()
}
...
fun getQuickFixes(file: CompiledFile, index: SymbolIndex, range: Range, diagnostics: List<Diagnostic>, filteredTypes: List<String> = emptyList()): List<Either<Command, CodeAction>> {
    return QUICK_FIXES.flatMap {
        it.compute(file, index, range, diagnostics, filteredTypes)
    }
}
