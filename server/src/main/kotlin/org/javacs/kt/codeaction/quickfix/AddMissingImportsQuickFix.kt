...
import org.javacs.kt.isTypeFiltered
...
class AddMissingImportsQuickFix : QuickFix {
    override fun compute(file: CompiledFile, index: SymbolIndex, range: Range, diagnostics: List<Diagnostic>, filteredTypes: List<String>): List<Either<Command, CodeAction>> {
        ...
        return unresolvedReferences.flatMap { diagnostic ->
            ...
            getImportAlternatives(symbolName, file.parse, index, filteredTypes).map { (importStr, edit) ->
    ...
    private fun getUnresolvedReferencesFromDiagnostics(diagnostics: List<Diagnostic>): List<Diagnostic> =
        diagnostics.filter {
            "UNRESOLVED_REFERENCE" == it.codeString()?.trim()
        }

    private fun getImportAlternatives(symbolName: String, file: KtFile, index: SymbolIndex, filteredTypes: List<String>): List<Pair<String, TextEdit>> {
        ...
        return queryResult
            ...
            .filter { !isTypeFiltered(it.fqName, filteredTypes) }
            .map {
