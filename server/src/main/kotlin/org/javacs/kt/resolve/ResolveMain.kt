...
private fun findCompanionObjectMain(file: KtFile): Pair<String?, TextRange>? = file.declarations
    ...
    .firstNotNullOfOrNull { companionObjectInternal ->
        ...
        val enclosingClass = it.parent?.parent?.parent?.parent
        check(enclosingClass is KtClass) {
            "Expected @JvmStatic main to be in 4 parent levels ending in KtClass, got ${enclosingClass?.javaClass?.simpleName}"
        }
        Pair(enclosingClass.fqName?.toString(), it.textRange)
    }
