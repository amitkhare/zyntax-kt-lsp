...
// TODO: Refactor - file has too many functions (20), consider splitting into CompiledFile + helpers
...
class CompiledFile(
    ...
    fun typeOfExpression(expression: KtExpression, scopeWithImports: LexicalScope): KotlinType? {
        // Fast path: try cached compile context by position
        // The expression comes from a virtual file created by parseAtPoint, but its
        // position matches the original parse when content hasn't diverged.
        if (content == parse.text) {
            val originalExpr = parse.findElementAt(expression.textRange.startOffset)
                ?.findParent<KtExpression>()
            if (originalExpr != null && originalExpr.text == expression.text) {
                val cachedType = compile.getType(originalExpr)
                if (cachedType != null) return cachedType
            }
        }
        // Slow path: full expression compilation (creates fresh compiler container)
    ...
    fun bindingContextOf(expression: KtExpression, scopeWithImports: LexicalScope): BindingContext? =
        try {
            ...
            LOG.warn("Error compiling expression for binding context: {}", e.message)
    ...
    private fun expandForType(cursor: Int, surroundingExpr: KtExpression): KtExpression {
        ...
        return if (dotParent != null && dotParent.selectorExpression?.textRange?.contains(cursor) ?: false) {
     ...
     * It primarily uses an expression-specific binding context.
     * Falls back to full compile only if fast path returns null (not if scope/context missing).
     */
    fun referenceAtPoint(cursor: Int): Pair<KtExpression, DeclarationDescriptor>? {
        val element = parseAtPoint(cursor, asReference = true)
        val cursorExpr = element?.findParent<KtExpression>()
        ...
        val result = referenceFromContext(cursor, path, context)
        if (result != null) return result

        // Only fall back if fast path returned null (reference not found in local scope)
        // Try full compile which includes JDK symbols
        LOG.debug("referenceAtPoint: fast path returned null, trying full compile")
    ...
    fun findDeclaration(cursor: Int): Pair<KtNamedDeclaration, Location>? {
        try {
            findDeclarationReference(cursor)?.let { return it }
        } catch (e: Exception) {
            LOG.debug { "findDeclarationReference failed: ${e.message}" }
            LOG.printStackTrace(e)
        }
        try {
            return findDeclarationCursorSite(cursor)
        } catch (e: Exception) {
            LOG.debug { "findDeclarationCursorSite failed: ${e.message}" }
            LOG.printStackTrace(e)
        }
        return null
    }
    ...
    private fun findDeclarationCursorSite(cursor: Int): Pair<KtNamedDeclaration, Location>? {
        val oldCursor = oldOffset(cursor)
        val element = parse.findElementAt(oldCursor) ?: return null
        val declaration = element.findParent<KtNamedDeclaration>() ?: return null

        val nameIdentifier = declaration.nameIdentifier ?: return null

        return Pair(
            declaration,
            Location(declaration.containingFile.toURIString(),
