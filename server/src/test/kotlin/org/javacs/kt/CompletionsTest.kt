...
class EditCallTest : SingleFileTestFixture("completions", "EditCall.kt") {
    @Test fun `edit existing function`() {
        ...
        assertThat(completions.items.find { it.label.startsWith("println") }, hasProperty("insertText", containsString("println")))
    }
}
...
class AutoImportWhitespaceTest : SingleFileTestFixture("completions", "AutoImportWhitespace.kt") {
    @Test fun `completion runs without error when cursor is on whitespace`() {
        // When cursor is on whitespace (not on a specific element), the completion
        // should still return results without throwing an error
        val completions = languageServer.textDocumentService.completion(completionParams(file, 3, 4)).get().right!!
        assertThat("Should return completion list without error", completions, notNullValue())
    }
}
