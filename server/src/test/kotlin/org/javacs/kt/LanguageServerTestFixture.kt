...
abstract class LanguageServerTestFixture(
    ...
    private fun createLanguageServer(config: Configuration): KotlinLanguageServer {
        ...
        languageServer.config.cache.workspaceCacheEnabled = false
        languageServer.connect(this)
        languageServer.initialize(init).join()
        languageServer.initialized(InitializedParams())

        // Wait for the initial lint/compilation submitted by initialize() to
        // complete before any test runs, ensuring CliTraceHolder.module etc.
        // are initialized (avoids races with async debouncer compilation).
