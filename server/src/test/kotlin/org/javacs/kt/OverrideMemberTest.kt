...
class OverrideMemberTest : SingleFileTestFixture("overridemember", "OverrideMembers.kt") {
    ...
    fun `should find members in jdk object`() {
        ...
        // Verify that Thread-specific methods are found (Java 21 includes virtual thread methods)
        // Use containsString matchers to be flexible about parameter names (cl vs p0) in different JDK versions
        val essentialMethods = listOf(
            "equals", "hashCode", "toString", "run", "clone", "start()",
            "interrupt", "isInterrupted", "countStackFrames",
            "getContextClassLoader", "setContextClassLoader", "getStackTrace",
            "getId", "getState", "getUncaughtExceptionHandler"
        )

        essentialMethods.forEach { method ->
            assertThat(titles, hasItem(containsString("override fun $method")))
        }

        // Verify edit texts contain key methods
        val padding = System.lineSeparator() + System.lineSeparator() + "    "
        listOf("equals", "hashCode", "toString").forEach { method ->
            assertThat(newTexts, hasItem(containsString(padding + "override fun $method")))
        }
    ...
    @Test
    fun `should detect nullable parameter as correctly implemented`() {
        // TestNullableOverride correctly implements String? parameter
        // Should show NO more overrides since it's already implemented
        val result = languageServer.getProtocolExtensionService().overrideMember(TextDocumentPositionParams(TextDocumentIdentifier(fileUri), position(41, 8))).get()

        assertThat(result, hasSize(0))
    }

    @Test
    fun `should detect Any nullable parameter correctly`() {
        // TestAnyParam correctly implements Any? parameter - should show NO more overrides
        val result = languageServer.getProtocolExtensionService().overrideMember(TextDocumentPositionParams(TextDocumentIdentifier(fileUri), position(47, 8))).get()
